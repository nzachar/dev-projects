...
const poiReducer = createReducer(
  initialState,
  on(PoiActions.loadPois, (state) => ({ ...state, loaded: false, error: null })),
  on(PoiActions.loadPoisSuccess, (state, { pois: poi }) =>
    poiAdapter.setAll(poi, { ...state, loaded: true })
  ),
  on(PoiActions.loadPoisFailure, (state, { error }) => ({ ...state, error })),
